  ...
  let build_function_body fdecl =
    ...
    let rec expr builder ((_, e) : sexpr) = match e with
      ...
      | SArrAssign (s, e1, e2) -> let a = expr builder e1 and b = expr builder e2 in
            (let getTheElementPtr = L.build_gep (lookup s) [|L.const_int i32_t 0; a|] s builder in L.build_store b getTheElementPtr builder)           
	...
	let getTheElementPtr = L.build_gep (lookup s) [|L.const_int i32_t 0; a; b|] s builder in L.build_store  c getTheElementPtr builder 
      ...
      | SBinop ((A.Int,_ ) as e1, op, e2) ->
         ...
         | A.Geq     -> L.build_icmp L.Icmp.Sge
          | A.And | A.Or ->
              raise (Failure "internal error: semant should have rejected and/or on Int")
          | _ ->  raise (Failure "error: not a viable int to int operation")
          ) e1' e2' "tmp" builder
